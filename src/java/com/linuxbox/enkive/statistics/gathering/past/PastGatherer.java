...
public abstract class PastGatherer {
	...
	// TODO
	public PastGatherer(String name, StatsClient client, int hrKeepTime,
			int dayKeepTime, int weekKeepTime, int monthKeepTime) {
		this.client = client;
	...
	protected Date getEarliestStatDate() {
		StatsQuery query = new MongoStatsQuery(gathererName, STAT_INTERVAL,
		...
		for (Map<String, Object> statMap : client.queryStatistics(query)) {
			if (statMap.get(CONSOLIDATION_TYPE) != null) {
				Map<String, Object> tsMap = (Map<String, Object>) statMap
						.get(STAT_TIMESTAMP);
	...
	public List<Map<String, Object>> consolidatePast(int grain, Calendar c) {
		...
		while (c.getTimeInMillis() > endDate.getTime()) {
			...
			try {
				Map<String, Object> consolidated = getConsolidatedData(start,
						end, grain);
				...
				LOGGER.error("Consolidated gatherer error on range " + start
						+ " to " + end, e);
			}
		}
		return result;
	...
	protected abstract Map<String, Object> getConsolidatedData(Date start,
			Date end, int grain) throws GathererException;
}
