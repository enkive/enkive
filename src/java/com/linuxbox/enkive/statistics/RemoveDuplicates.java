		...
		while (queryStartTime.before(endTimestamp)) {
			...
			if (cursor.count() > 0) {
				...
				for (DBObject statDataObj : cursor) {
					...
					shouldBeKept = true;
					String gathererName = statData.get(STAT_GATHERER_NAME)
							.toString();

					// check to see if already have that gatherer's data
					for (Map<String, Object> uniqueStatData : dataToKeep) {
						String uniqueStatName = uniqueStatData.get(
								STAT_GATHERER_NAME).toString();
					...
					if (shouldBeKept) {
						Map<String, Object> statDataToKeep = new HashMap<String, Object>(
								statData);
