...
public abstract class ImapRequestLineReader {
	 ...
	 * @param extraCRLF
	 *            <code>true</code> if extra CRLF is wanted, <code>false</code>
	 *            else
	 * @throws DecodingException
	 *             If a char can't be read into each array element.
	 */
	public abstract InputStream read(int size, boolean extraCRLF)
			throws DecodingException;
	 ...
	 */
	protected abstract void commandContinuationRequest()
			throws DecodingException;
	...
	/**
	 * 
	 * Reads the mailbox name via {@link #mailboxUTF7()} but also decode it via
	 * {@link CharsetUtil#decodeModifiedUTF7(String)}
	 * 
	 ...
	 */
	public String consumeLiteral(final Charset charset)
			throws DecodingException {
		if (charset == null) {
	...
	public InputStream consumeLiteral(boolean extraCRLF)
			throws DecodingException {
	 ...
	 * 
	 * @param stopOnParen
	 *            true if it should stop to parse on the first closing paren
	 * @return number
	 */
	...
	private long readDigits(int add, final long total, final boolean first,
			boolean stopOnParen) throws DecodingException {
		final char next;
		if (first) {
			next = nextWordChar();
		} else {
	...
	/**
	 * Reads a "message set" argument, and parses into an IdSet. This also
	 * support the use of $ as sequence-set as stated in SEARCHRES RFC5182
	 */
	...
	private IdRange parseRange(String range) throws DecodingException {
		...
		try {
			...
			throw new DecodingException(HumanReadableText.INVALID_MESSAGESET,
					"Invalid message set.", e);
		}
	}
	...
	private long parseUnsignedInteger(String value) throws DecodingException {
		if (value.length() == 1 && value.charAt(0) == '*') {
			...
			if (number < ImapConstants.MIN_NZ_NUMBER
					|| number > ImapConstants.MAX_NZ_NUMBER)
				throw new DecodingException(
						HumanReadableText.INVALID_MESSAGESET,
						"Invalid message set. Numbers must be unsigned 32-bit Integers");
			return number;
